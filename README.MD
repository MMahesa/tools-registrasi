...
window.APP_CONFIG = {
    odpOptions: [
        { value: 'EXISTING_ODP', label: 'EXISTING_ODP', vlan: 10 },
        { value: 'NEW_LOCATION', label: 'NEW_LOCATION', vlan: 20 }
    ]
};
```
...
Update `paketOptions` di `config.js`:
```javascript
window.APP_CONFIG = {
    paketOptions: [
        'Paket UP TO 25 Mbps',
...
```
tools-registrasi/
├── ⚙️ config.js           # Data ODP, paket, dan quick access
